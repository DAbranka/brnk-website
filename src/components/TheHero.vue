...
<template>
  <header>
    <!-- NAVBAR -->
    <nav class="navbBar w-full">
      <section
        class="navBar__content wrapper flex flex-row items-center justify-between mx-40 py-2"
      >
        <!-- LOGO -->
        <RouterLink to="/">
          <img
            class="logo h-5"
            src="@/assets/img/BRNK_LOGO_WHT_PNG_NOBG.png"
            alt="png/logo"
          />
        </RouterLink>
    ...
    <section class="subTitle w-full flex items-end absolute top-0 h-full -z-10">
      <div class="subTitle__content" flex items-center w-full px-40>
        <!-- <p class="w-20rem py-10">DIGITAL CREATOR</p> -->
        <!-- put a white line in tailwindcss -->
        <!-- <div id="hr-wrap" w-full>
          <hr class="border-1" />
        </div> -->
        <!-- <p w-20rem flex justify-end>WEB DEVELOPER</p> -->
      </div>
    </section>

    <!-- About Card -->
    <AboutCard
      v-if="showAboutCard.value"
      :onClose="() => (showAboutCard.value = false)"
    />
  </header>
</template>
